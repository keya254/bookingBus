...
trait DatesOwner
{
    ...
    public function new_passengers_for_owners($type, $date = null): array
    {
        ...
        $passenger = array_unique(Seat::whereIn('trip_id', $trips)->where('passenger_id', '!=', null)->pluck('passenger_id')->toArray());

        $$variable = Passenger::whereIn('id', $passenger)->$condition('created_at', $date ?? now())->count();
