...
trait DatesAdmin
{
     ...
     * @param $date of date can be null if null using now() date
     **/
    public function new_type_cars($type, $date = null): array
    {
        //check if type correct and in this ['year','month','date']
        $this->check_type_date($type);

        $variable = "new_type_cars_in_" . strtolower($type);
    ...
    public function all_admin_date($type, $date = null): array
    {
        ...
        $this->new_type_cars($type, $date) + $this->new_trips($type, $date) + $this->new_passengers($type, $date);
    }
