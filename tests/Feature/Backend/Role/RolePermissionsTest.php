...
class RolePermissionsTest extends TestCase
{
    ...
    {
        $this->get(route('get-role-permissions', $this->role->id), array('HTTP_X-Requested-With' => 'XMLHttpRequest'))
    ...
    public function test_not_has_permission_role_permission_get_permissions_to_role()
    {
        $this->actingAs($this->user2)
            ->get(route('get-role-permissions', $this->role->id), array('HTTP_X-Requested-With' => 'XMLHttpRequest'))
            ->assertForbidden();
    }
    ...
    {
        $this->actingAs($this->user)
            ->get(route('get-role-permissions', $this->role->id + 23), array('HTTP_X-Requested-With' => 'XMLHttpRequest'))
            ->assertStatus(500);
    }
    ...
    {
        $this->actingAs($this->user)
            ->json('post', route('role-permissions'), ['role_id' => 1, 'permissions' => [1]])
            ->assertSuccessful();
    }

    public function test_guest_set_permissions_to_role()
    {
        $this->json('post', route('role-permissions'), ['role_id' => 1, 'permissions' => [1]])
            ->assertUnauthorized();
    }
    ...
    {
        $this->actingAs($this->user)
            ->json('post', route('role-permissions'), ['role_id' => 2, 'permissions' => [1]])
            ->assertStatus(500);
    }
    ...
    {
        $this->actingAs($this->user)
            ->json('post', route('role-permissions'), ['role_id' => 2, 'permissions' => [1, 2]])
            ->assertStatus(500);
    }
    ...
    {
        $this->actingAs($this->user)
            ->json('post', route('role-permissions'), ['role_id' => '', 'permissions' => [1]])
    ...
    {
        $this->actingAs($this->user)
            ->json('post', route('role-permissions'), ['role_id' => 1, 'permissions' => ''])
    ...
    {
        $this->actingAs($this->user)
            ->json('post', route('role-permissions'), ['role_id' => 1, 'permissions' => 'moh'])
            ->assertJsonValidationErrors('permissions')
            ->assertStatus(422);
    }
    ...
    {
        $this->actingAs($this->user)
            ->json('post', route('role-permissions'), ['role_id' => 1, 'permissions' => ['mi', 'htgty', 1]])
